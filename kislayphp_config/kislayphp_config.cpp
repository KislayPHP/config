...
ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_kislayphp_config_refresh, 0, 0, _IS_BOOL, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_kislayphp_config_get_int, 0, 1, IS_LONG, 0)
    ZEND_ARG_TYPE_INFO(0, key, IS_STRING, 0)
    ZEND_ARG_TYPE_INFO_WITH_DEFAULT_VALUE(0, default, IS_LONG, 0, "0")
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_kislayphp_config_get_bool, 0, 1, _IS_BOOL, 0)
    ZEND_ARG_TYPE_INFO(0, key, IS_STRING, 0)
    ZEND_ARG_TYPE_INFO_WITH_DEFAULT_VALUE(0, default, _IS_BOOL, 0, "false")
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_kislayphp_config_get_array, 0, 1, IS_ARRAY, 0)
    ZEND_ARG_TYPE_INFO(0, key, IS_STRING, 0)
    ZEND_ARG_TYPE_INFO_WITH_DEFAULT_VALUE(0, default, IS_ARRAY, 0, "[]")
ZEND_END_ARG_INFO()

PHP_METHOD(KislayPHPConfig, refresh) {
    ZEND_PARSE_PARAMETERS_NONE();
    ...
    zend_string *str_key;
    zval *entry;
    ZEND_HASH_FOREACH_STR_KEY_VAL(Z_ARRVAL(retval), str_key, entry) {
        if (str_key != nullptr && Z_TYPE_P(entry) == IS_STRING) {
            obj->values[std::string(ZSTR_VAL(str_key), ZSTR_LEN(str_key))] =
                std::string(Z_STRVAL_P(entry), Z_STRLEN_P(entry));
        }
...
PHP_METHOD(KislayPHPConfig, getInt) {
    char *key = nullptr;
    size_t key_len = 0;
    ...
    if (obj->has_client) {
        ...
        if (!Z_ISUNDEF(retval) && Z_TYPE(retval) != IS_NULL) {
            if (Z_TYPE(retval) == IS_STRING) {
                value = std::string(Z_STRVAL(retval), Z_STRLEN(retval));
                found = true;
    ...
    char *end = nullptr;
    zend_long result = static_cast<zend_long>(std::strtoll(value.c_str(), &end, 10));
    if (end == value.c_str()) {
        RETURN_LONG(default_val);
    }
    RETURN_LONG(result);
...
PHP_METHOD(KislayPHPConfig, getBool) {
    char *key = nullptr;
    size_t key_len = 0;
...
PHP_METHOD(KislayPHPConfig, getArray) {
    char *key = nullptr;
    size_t key_len = 0;
    zval *default_val = nullptr;
    ZEND_PARSE_PARAMETERS_START(1, 2)
    ...
    zval decoded;
    ZVAL_UNDEF(&decoded);
    zend_string *json_str = zend_string_init(value.c_str(), value.size(), 0);
    if (php_json_decode_ex(&decoded, ZSTR_VAL(json_str), ZSTR_LEN(json_str), PHP_JSON_OBJECT_AS_ARRAY, 512) == SUCCESS
        && Z_TYPE(decoded) == IS_ARRAY) {
        zend_string_release(json_str);
        RETVAL_ZVAL(&decoded, 0, 0);
        return;
    }
    zend_string_release(json_str);
...
static const zend_function_entry kislayphp_config_methods[] = {
    ...
    PHP_ME(KislayPHPConfig, refresh, arginfo_kislayphp_config_refresh, ZEND_ACC_PUBLIC)
    PHP_ME(KislayPHPConfig, getInt, arginfo_kislayphp_config_get_int, ZEND_ACC_PUBLIC)
    PHP_ME(KislayPHPConfig, getBool, arginfo_kislayphp_config_get_bool, ZEND_ACC_PUBLIC)
    PHP_ME(KislayPHPConfig, getArray, arginfo_kislayphp_config_get_array, ZEND_ACC_PUBLIC)
    PHP_FE_END
};
